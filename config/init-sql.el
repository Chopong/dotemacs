...
(defun fix-postgres-prompt-regexp ()
...
(add-hook 'sql-interactive-mode-hook 'fix-postgres-prompt-regexp)
...
(after-load 'sql
  ...
  (when (package-installed-p 'dash-at-point)
    (defun maybe-set-dash-db-docset (&rest _)
      (when (eq sql-product 'postgres)
    ...
    (advice-add 'sql-set-product :after 'maybe-set-dash-db-docset)))

(setq-default sql-input-ring-file-name
              (expand-file-name ".sqli_history" user-assets-directory))
